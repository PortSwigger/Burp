...
public class BurpExtender extends AbstractTableModel implements IBurpExtender, ITab, ActionListener
{
    ...
    public void registerExtenderCallbacks(final IBurpExtenderCallbacks callbacks)
    {
        ...
        SwingUtilities.invokeLater(new Runnable() 
        {
            ...
            public void run()
            {
                ...
                //runButton.setPreferredSize(new Dimension(130,30));
                //splitPane.setTopComponent(btnRun);
                topPanel.add(btnRun);
	        ...
	        for (IHttpRequestResponse rr : callbacks.getProxyHistory()) {
	            try {
	                ...
	                if (callbacks.isInScope(url)) {
	                    ...
	                    List<IParameter> params = helpers.analyzeRequest(rr).getParameters();
	                    if (!params.isEmpty())
	                    {
	                    ...
	                    int row = log.size();
	                    log.add(new LogEntry(protocol, host, port, url, parameters, cookie));
    ...
    private void writeFile() {
		...
		try {
    		...
    		outputFile = new BufferedWriter(new FileWriter(aFile, aFile.exists()));
		}
		...
		try {
			outputFile.write("Protocol, Host, Port, Path, Page, Parameters\n");
			...
			for (LogEntry le : log) {
				try {
					...
					// Remove host
					String path = le.url.toString().replace(le.protocol + "://" + le.host + ":" + le.port, "");
...
//        }        
    ...
    private static class LogEntry
    {
        ...
        LogEntry(String protocol, String host, int port, URL url, String params, String cookie)
        {
